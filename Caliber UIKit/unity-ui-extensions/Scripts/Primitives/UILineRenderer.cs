   ...
   public class UILineRenderer : UIPrimitiveBase
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            if (LineList)
            {
                for (var i = 1; i < pointsToDraw.Length; i += 2)
                {
                    ...
                    if (LineCaps)
                    {
                        segments.Add(CreateLineCap(i, start, end, SegmentType.Start));
                    }

                    segments.Add(CreateLineSegment(i, start, end, SegmentType.Middle));

                    if (LineCaps)
                    {
                        segments.Add(CreateLineCap(i, start, end, SegmentType.End));
                    }
                }
            }
            else
        ...
        private UIVertex[] CreateLineCap(Int32 idx, Vector2 start, Vector2 end, SegmentType type)
        {
            if (type == SegmentType.Start)
            {
                var capStart = start - ((end - start).normalized * LineThickness / 2);
                return CreateLineSegment(idx, capStart, start, SegmentType.Start);
            }
            else if (type == SegmentType.End)
            {
                var capEnd = end + ((end - start).normalized * LineThickness / 2);
                return CreateLineSegment(idx, end, capEnd, SegmentType.End);
            }
        ...
        private UIVertex[] CreateLineSegment(Int32 idx, Vector2 start, Vector2 end, SegmentType type)
        {
            var uvs = middleUvs;
            if (type == SegmentType.Start)
                uvs = startUvs;
            ...
            var v3 = end + offset;
            var v4 = end - offset;
            return SetVbo(idx, new[] { v1, v2, v3, v4 }, uvs);
        }

        protected UIVertex[] SetVbo(Int32 idx, Vector2[] vertices, Vector2[] uvs)
        {
            UIVertex[] vbo = new UIVertex[4];
            for (int i = 0; i < vertices.Length; i++)
            {
                var vert = UIVertex.simpleVert;
                vert.color = GetGradientColor(idx);
        ...
        private Color GetGradientColor(Int32 idx)
        {
            return Gradient.Evaluate((Single)idx / Points.Length);
        }
    }
}