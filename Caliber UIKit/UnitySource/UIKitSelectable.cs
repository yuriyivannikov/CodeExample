...
{
    /// <summary>
    /// Является копией класса UnityEngine.UI.Selectable с небольшими правками
    /// https://bitbucket.org/Unity-Technologies/ui/src/f0c70f707cf09f959ad417049cb070f8e296ffe2/UnityEngine.UI/UI/Core/Selectable.cs?at=5.5&fileviewer=file-view-default
    /// </summary>
        ...
        IDeselectHandler
    {
        ...
        private static List<UIKitSelectable> s_List = new List<UIKitSelectable>();
        public static List<UIKitSelectable> allSelectables { get { return s_List; } }

        // Navigation information.
        [FormerlySerializedAs("navigation")]
        [SerializeField]
        private UIKitNavigation m_Navigation = UIKitNavigation.defaultNavigation;
...
//TODO:Fix it        
//        private StatisticsTrackerComponent m_StatisticsTracker;

        [SerializeField]
        protected PointerEventData.InputButton m_InteractableKey = PointerEventData.InputButton.Left;
        public PointerEventData.InputButton InteractableKey => m_InteractableKey;

        private bool m_GroupsAllowInteraction = true;

        private SelectionState m_CurrentSelectionState;

        public UIKitNavigation navigation { get { return m_Navigation; } set { if (UIKitSetPropertyUtility.SetStruct(ref m_Navigation, value)) OnSetProperty(); } }
        public Transition transition { get { return m_Transition; } set { if (UIKitSetPropertyUtility.SetStruct(ref m_Transition, value)) OnSetProperty(); } }
        public ColorBlock colors { get { return m_Colors; } set { if (UIKitSetPropertyUtility.SetStruct(ref m_Colors, value)) OnSetProperty(); } }
        public SpriteState spriteState { get { return m_SpriteState; } set { if (UIKitSetPropertyUtility.SetStruct(ref m_SpriteState, value)) OnSetProperty(); } }
        public AnimationTriggers animationTriggers { get { return m_AnimationTriggers; } set { if (UIKitSetPropertyUtility.SetClass(ref m_AnimationTriggers, value)) OnSetProperty(); } }
        public Graphic targetGraphic { get { return m_TargetGraphic; } set { if (UIKitSetPropertyUtility.SetClass(ref m_TargetGraphic, value)) OnSetProperty(); } }
        public bool interactable
        {
            get { return m_Interactable; }
            set
            {
                if (UIKitSetPropertyUtility.SetStruct(ref m_Interactable, value))
                {
...
//TODO:Fix it
//        public StatisticsTrackerComponent statisticsTracker { get { return m_StatisticsTracker; } set { if (UIKitSetPropertyUtility.SetClass(ref m_StatisticsTracker, value)) OnSetProperty(); } }//статистика

        protected SelectionState _lastState = SelectionState.Normal;
        private bool isPointerInside { get; set; }
        private bool isPointerDown { get; set; }
        private bool hasSelection { get; set; }

        protected UIKitSelectable()
        { }
...
//            {
//                m_StatisticsTracker = gameObject.GetComponent(typeof (StatisticsTrackerComponent)) as StatisticsTrackerComponent;
//
//                if (m_StatisticsTracker == null)
//                    m_StatisticsTracker = gameObject.AddComponent(typeof (StatisticsTrackerComponent)) as StatisticsTrackerComponent;
//            }
        }
        ...
        public void UpdateTransitions(bool instant = true)
        {
            DoStateTransition(_lastState, instant);
        }
        ...
        public UIKitSelectable FindSelectable(Vector3 dir)
        {
            dir = dir.normalized;
            Vector3 localDir = Quaternion.Inverse(transform.rotation) * dir;
            Vector3 pos = transform.TransformPoint(GetPointOnRectEdge(transform as RectTransform, localDir));
            float maxScore = Mathf.NegativeInfinity;
            UIKitSelectable bestPick = null;
            for (int i = 0; i < s_List.Count; ++i)
            {
                UIKitSelectable sel = s_List[i];

                if (sel == this || sel == null)
                    continue;

                if (!GraphicRaycasterEnabled(sel))
                    continue;

                if (!sel.IsInteractable() || sel.navigation.mode == UIKitNavigation.Mode.None)
                    continue;
        ...
        public bool GraphicRaycasterEnabled(UIKitSelectable sel)
        {
            var graphicRaycaster = sel.transform.root.GetComponent<GraphicRaycaster>();

            if (graphicRaycaster == null)
                graphicRaycaster = sel.transform.GetComponentInParent<GraphicRaycaster>();
        ...
        void Navigate(AxisEventData eventData, UIKitSelectable sel)
        {
        ...
        public virtual UIKitSelectable FindSelectableOnLeft()
        {
            if (m_Navigation.mode == UIKitNavigation.Mode.Explicit)
            {
                return m_Navigation.selectOnLeft;
            }
            if ((m_Navigation.mode & UIKitNavigation.Mode.Horizontal) != 0)
            {
        ...
        public virtual UIKitSelectable FindSelectableOnRight()
        {
            if (m_Navigation.mode == UIKitNavigation.Mode.Explicit)
            {
        ...
        public virtual UIKitSelectable FindSelectableOnUp()
        {
            if (m_Navigation.mode == UIKitNavigation.Mode.Explicit)
            {
                return m_Navigation.selectOnUp;
            }
            if ((m_Navigation.mode & UIKitNavigation.Mode.Vertical) != 0)
            {
        ...
        public virtual UIKitSelectable FindSelectableOnDown()
        {
            if (m_Navigation.mode == UIKitNavigation.Mode.Explicit)
            {
        ...
        protected bool IsHighlighted(BaseEventData eventData)
        {
            ...
            bool isHighlighted = false;// hasSelection, отключено отображение фокуса через состояние наведения
        ...
        public virtual void OnPointerDown(PointerEventData eventData)
        {
            ...
            // Selection tracking
            if (IsInteractable() && navigation.mode != UIKitNavigation.Mode.None && EventSystem.current != null)
